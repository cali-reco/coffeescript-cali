...
var Circle, Command, Delete, Evaluate, FuzzySet, Gesture, Helper, Line, List, Polygon, Recognizer, Rectangle, Scribble, Shape, Stroke, Tap, Unknown, Vector, _ref, _ref1, _ref2,
    ...
    this.__defineGetter__("perimeter", function() {
      ...
      if (this._perimeter === 0) {
        for (i = _i = 0, _ref = this.length - 1; 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
          this._perimeter += Helper.distance(this[i], this[i + 1]);
        }
        if (this.length < 3) {
          this._perimeter *= 2;
        }
...
Scribble = (function(_super) {
  ...
  function Scribble(strokes) {
    var point, s, stroke, _i, _j, _len, _len1, _ref1, _ref2, _ref3;
    if (strokes == null) {
      strokes = [];
    }
    ...
    for (_i = 0, _len = strokes.length; _i < _len; _i++) {
      ...
      if (!(stroke instanceof Stroke)) {
        ...
        for (_j = 0, _len1 = stroke.length; _j < _len1; _j++) {
          point = stroke[_j];
          s.push((_ref1 = point.x) != null ? _ref1 : point[0], (_ref2 = point.y) != null ? _ref2 : point[1], (_ref3 = point.time) != null ? _ref3 : point[2]);
        }
        stroke = s;
      }
      this.push(stroke);
    ...
    this.__defineGetter__("largestQuad", function() {
      return this._largestQuad || this.getLargestQuad();
    });
  ...
  Scribble.prototype.clone = function() {
    var newStroke, point, scribble, stroke, _i, _j, _len, _len1;
  ...
  */
  ...
  Scribble.prototype.getBoundingBox = function() {
    var point, x1, x2, y1, y2, _i, _len, _ref1;
    x1 = x2 = this.convexHull[0].x;
    y1 = y2 = this.convexHull[0].y;
    _ref1 = this.convexHull;
  ...
  */
  ...
  Scribble.prototype.getConvexHull = function() {
    var filter, i, np, ordPoints, ordedPoints, pt, strokes, tmp, _i, _len;
    strokes = this;
    ordPoints = function(min) {
      var ang, ordedPoints, point, ptr, stroke, _i, _j, _len, _len1;
    ...
    filter = function(polygon) {
      var convexHull, i, point, prev, _i, _len;
      convexHull = new Polygon();
      for (i = _i = 0, _len = polygon.length; _i < _len; i = ++_i) {
        point = polygon[i];
        if (i === 0) {
          prev = point;
          continue;
    ...
    for (i = _i = 0, _len = ordedPoints.length; _i < _len; i = ++_i) {
      pt = ordedPoints[i];
      if (i >= 1) {
  ...
  Scribble.prototype.findLowest = function() {
    var min, point, stroke, _i, _j, _len, _len1;
    min = this[0][0];
    for (_i = 0, _len = this.length; _i < _len; _i++) {
      stroke = this[_i];
      for (_j = 0, _len1 = stroke.length; _j < _len1; _j++) {
  ...
  */
  ...
  Scribble.prototype.getEnclosingRect = function() {
    var M_PI_2, a, ang, area, dis, i, maxx, maxxp, maxy, maxyp, min_area, minx, minxp, miny, minyp, num, p1, p1x, p1y, p2, p2x, p2y, p3, p3x, p3y, p4, p4x, p4y, paux, paux1, paux2, v1, v2, v3, v4, xx, yy, _i, _j, _ref1;
  ...
  Scribble.prototype.getPtsInSmallTri = function() {
    var dx, dy, empty, i, inter, k, m, pt, stroke, tri, x, _, _i, _j, _k, _l, _len, _len1;
    tri = this.smallTriangle;
    m = [
      (function() {
        var _i, _results;
        _results = [];
        for (_ = _i = 0; _i < 3; _ = ++_i) {
          _results.push(0);
        }
        return _results;
      })()
    ...
    for (_j = 0, _len = this.length; _j < _len; _j++) {
      stroke = this[_j];
      for (_k = 0, _len1 = stroke.length; _k < _len1; _k++) {
        pt = stroke[_k];
        if (pt.x >= tri[0].x && pt.x >= tri[1].x && pt.x >= tri[2].x) {
          continue;
        }
        if (pt.x <= tri[0].x && pt.x <= tri[1].x && pt.x <= tri[2].x) {
          continue;
        }
        if (pt.y >= tri[0].y && pt.y >= tri[1].y && pt.y >= tri[2].y) {
          continue;
        }
        if (pt.y <= tri[0].y && pt.y <= tri[1].y && pt.y <= tri[2].y) {
          continue;
  ...
  Scribble.prototype.getSmallTriangle = function() {
    var m1, m2, m3, p1, p2, p3, t1, t2, t3, _ref1;
    _ref1 = this.largestTriangle, p1 = _ref1[0], p2 = _ref1[1], p3 = _ref1[2];
    m1 = {
      x: p3.x + (p1.x - p3.x) / 2,
      y: p3.y + (p1.y - p3.y) / 2
    };
  ...
  Scribble.prototype.compRootedTri = function(pts, ripa, ripb, ripc, np) {
    var area, ia, ib, ic, pa, pb, pc, trigArea, _i, _ref1;
    trigArea = 0;
    ia = ripa;
    ib = ripb;
    for (ic = _i = ripc, _ref1 = np - 1; ripc <= _ref1 ? _i < _ref1 : _i > _ref1; ic = ripc <= _ref1 ? ++_i : --_i) {
      pa = pts[ia];
      pb = pts[ib];
      pc = pts[ic];
      if ((ic < (np - 1)) && (area = Helper.triangleArea(pa, pb, pc)) > trigArea) {
        ripc = ic;
        trigArea = area;
      } else {
        break;
  ...
  Scribble.prototype.compRootedQuad = function(pts, ripa, ripb, ripc, ripd, np) {
    var area, id, pa, pb, pc, pd, trigArea, _i, _ref1;
    trigArea = 0;
    pa = pts[ripa];
    pb = pts[ripb];
    pc = pts[ripc];
    for (id = _i = ripd, _ref1 = np - 1; ripd <= _ref1 ? _i < _ref1 : _i > _ref1; id = ripd <= _ref1 ? ++_i : --_i) {
      pd = pts[id];
      if ((id < (np - 1)) && (area = Helper.quadArea(pa, pb, pc, pd)) > trigArea) {
        ripd = id;
        trigArea = area;
      } else {
        break;
    ...
    return [trigArea, ripd];
  };

  Scribble.prototype.getLargestTriangle = function() {
    var area, finalArea, fipa, fipb, fipc, i, ia, ib, ib0, ic, np, pf0, pf1, pf2, pt, pts, ripa, ripb, ripc, triArea, _i, _j, _k, _l, _len, _m, _ref1, _ref2, _ref3, _ref4, _ref5, _ref6;
    pts = this.convexHull;
    ...
    if (this.convexHull.length <= 3) {
      ...
      for (i = _j = _ref2 = this.convexHull.length; _ref2 <= 4 ? _j < 4 : _j > 4; i = _ref2 <= 4 ? ++_j : --_j) {
        this._largestTriangle.push(this.convexHull[0]);
      }
      return this._largestTriangle;
    }
    ia = area = triArea = 0;
    for (ib = _k = 1, _ref3 = this.convexHull.length - 2; 1 <= _ref3 ? _k <= _ref3 : _k >= _ref3; ib = 1 <= _ref3 ? ++_k : --_k) {
      if (ib >= 2) {
        ic = ib + 1;
      } else {
        ic = 2;
      }
      _ref4 = this.compRootedTri(pts, ia, ib, ic, np), area = _ref4[0], ic = _ref4[1];
      if (area > triArea) {
        triArea = area;
        ripa = ia;
        ripb = ib;
    ...
    pf1 = ripb;
    pf2 = ripc;
    for (ia = _l = _ref5 = ripa + 1; _ref5 <= ripb ? _l <= ripb : _l >= ripb; ia = _ref5 <= ripb ? ++_l : --_l) {
      triArea = 0;
      if (ia === ripb) {
        ib0 = ripb + 1;
      } else {
      ...
      area = 0;
      for (ib = _m = ib0; ib0 <= ripc ? _m <= ripc : _m >= ripc; ib = ib0 <= ripc ? ++_m : --_m) {
        if (ib === ripc) {
          ic = ripc + 1;
        } else {
          ic = ripc;
        }
        _ref6 = this.compRootedTri(pts, ia, ib, ic, np), area = _ref6[0], ic = _ref6[1];
        if (area > triArea) {
          triArea = area;
          fipa = ia;
          fipb = ib;
  ...
  Scribble.prototype.getLargestQuad = function() {
    var area, finalArea, fipa, fipb, fipc, fipd, i, ia, ib, ib0, ic, ic0, id, np, pf0, pf1, pf2, pf3, pts, quadArea, ripa, ripb, ripc, ripd, triArea, _i, _j, _k, _l, _m, _n, _o, _p, _ref1, _ref2, _ref3, _ref4, _ref5, _ref6;
    pts = this.convexHull;
    np = this.convexHull.length;
    ripa = ripb = ripc = 0;
    if (np <= 4) {
      this._largestQuad = new Polygon();
      for (i = _i = 0; 0 <= np ? _i < np : _i > np; i = 0 <= np ? ++_i : --_i) {
        this._largestQuad.push(pts[i]);
      }
      for (i = _j = np; np <= 5 ? _j < 5 : _j > 5; i = np <= 5 ? ++_j : --_j) {
        this._largestQuad.push(pts[0]);
      }
      return this._largestQuad;
    }
    ia = area = triArea = 0;
    for (ib = _k = 1, _ref1 = np - 2; 1 <= _ref1 ? _k <= _ref1 : _k >= _ref1; ib = 1 <= _ref1 ? ++_k : --_k) {
      if (ib >= 2) {
        ic = ib + 1;
      } else {
        ic = 2;
      }
      _ref2 = this.compRootedTri(pts, ia, ib, ic, np), area = _ref2[0], ic = _ref2[1];
      if (area > triArea) {
        triArea = area;
        ripa = ia;
        ripb = ib;
    ...
    fipa = fipb = fipc = fipd = 0;
    id = ib0 = quadArea = 0;
    quadArea = 0;
    for (ib = _l = _ref3 = ripa + 1; _ref3 <= ripb ? _l <= ripb : _l >= ripb; ib = _ref3 <= ripb ? ++_l : --_l) {
      if (ib === ripb) {
        ic0 = ripb + 1;
      } else {
        ic0 = ripb;
      }
      for (ic = _m = ic0; ic0 <= ripc ? _m <= ripc : _m >= ripc; ic = ic0 <= ripc ? ++_m : --_m) {
        if (ic === ripc) {
          id = ripc + 1;
        } else {
          id = ripc;
        }
        _ref4 = this.compRootedQuad(pts, ia, ib, ic, id, np), area = _ref4[0], id = _ref4[1];
        if (area > quadArea) {
          quadArea = area;
          fipa = ia;
          fipb = ib;
    ...
    ripc = fipc;
    ripd = fipd;
    for (ia = _n = _ref5 = ripa + 1; _ref5 <= ripb ? _n <= ripb : _n >= ripb; ia = _ref5 <= ripb ? ++_n : --_n) {
      if (ia === ripb) {
        ib0 = ripb + 1;
      } else {
        ib0 = ripb;
      ...
      quadArea = 0;
      area = 0;
      for (ib = _o = ib0; ib0 <= ripc ? _o <= ripc : _o >= ripc; ib = ib0 <= ripc ? ++_o : --_o) {
        if (ib === ripc) {
          ic0 = ripc + 1;
        } else {
          ic0 = ripc;
        }
        for (ic = _p = ic0; ic0 <= ripd ? _p <= ripd : _p >= ripd; ic = ic0 <= ripd ? ++_p : --_p) {
          if (ic === ripd) {
            id = ripd + 1;
          } else {
            id = ripd;
          }
          _ref6 = this.compRootedQuad(pts, ia, ib, ic, id, np), area = _ref6[0], id = _ref6[1];
          if (area > quadArea) {
            quadArea = area;
            fipa = ia;
            fipb = ib;
    ...
    return this._largestQuad = new Polygon(this.convexHull[pf0], this.convexHull[pf1], this.convexHull[pf2], this.convexHull[pf3], this.convexHull[pf0]);
  };
...
FuzzySet = (function() {
  ...
  FuzzySet.prototype.degOfMember = function(value) {
    if ((value < (this.a - this.wa)) || (value > (this.b + this.wb))) {
      return 0;
    }
...
Gesture.prototype.Features = function() {
  var a, awa, b, bwb, fn, fuzzySet, fuzzyVals, node, nodes, _fn, _i, _len, _list;
  nodes = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
  _list = [];
  _fn = function(fn, fuzzySet) {
    return _list.push(function(scribble) {
...
Tap = (function(_super) {
  ...
  function Tap() {
    Tap.__super__.constructor.call(this);
    this.name = "Tap";
    this.point = {
      x: 0,
      y: 0
...
Unknown = (function(_super) {
  ...
  Unknown.prototype.setUp = function(scribble) {};
...
Rectangle = (function(_super) {
  ...
  function Rectangle(rotated) {
    ...
    this.name = "Rectangle";
    if (rotated) {
      this._features = Gesture.prototype.Features([Evaluate.Ach_Aer, [0.75, 0.85, 1.0, 1.0]], [Evaluate.Alq_Aer, [0.72, 0.78, 1.0, 1.0]], [Evaluate.Hollowness, [0.0, 0.0, 1.0, 1.0]]);
    } else {
      this._features = Gesture.prototype.Features([Evaluate.Ach_Abb, [0.8, 0.83, 1.0, 1.0]], [Evaluate.Pch_Pbb, [0.87, 0.9, 1.0, 1.0]], [Evaluate.Alt_Abb, [0.45, 0.47, 0.5, 0.52]]);
    }
  }
...
Circle = (function(_super) {
  ...
  function Circle(rotated) {
    ...
    this._features = Gesture.prototype.Features([Evaluate.Pch2_Ach, [12.5, 12.5, 13.2, 13.5]], [Evaluate.Hollowness, [0.0, 0.0, 0.0, 0.0]]);
  }
...
Delete = (function(_super) {
  ...
  function Delete() {
    ...
    this._features = Gesture.prototype.Features([Evaluate.Her_Wer, [0.06, 0.08, 1.0, 1.0]], [Evaluate.Tl_Pch, [1.5, 1.9]], [Evaluate.Hollowness, [0.0, 3.0]], [Evaluate.Ns, [1.0, 1.0, 1.0, 1.0]]);
  }

  Delete.prototype.setUp = function(scribble) {
    this.point = scribble.startingPoint;
    this.dom = 1;
    return this.scribble = scribble;
  };
...
(typeof exports !== "undefined" && exports !== null ? exports : this).Delete = Delete;

if (typeof WEB === "undefined" || WEB === null) {
  List = require("./list").List;
  Scribble = require("./scribble").Scribble;
  _ref1 = require("./shape"), Shape = _ref1.Shape, Unknown = _ref1.Unknown;
...
Recognizer = (function() {
  ...
  Recognizer.prototype.addAllShapes = function(rotated) {
    this._shapesList = [];
    return this._shapesList.push(new Delete(), new Line(rotated), new Rectangle(rotated), new Circle(rotated));
  };

  Recognizer.prototype.recognize = function(scribble) {
    var name, shape, shapes, val, val2, _i, _j, _len, _len1, _ref3, _ref4;
    if (!(scribble instanceof Scribble)) {
